...
use crate::picture::resolution::GetResolution;
use crate::picture::Picture;
use crate::picture::ValidRef;
use crate::scoring::VmafScoring;
...
pub struct Vmaf(*mut VmafContext, Box<dyn VmafScoring>);
...
impl Vmaf {
    ...
    pub fn load_model(&mut self, model: impl VmafScoring) {
        model.load(self);
    }
    ...
    pub fn get_vmaf_scores<
        I: GetResolution
            + ExactSizeIterator
            + Iterator<Item = impl TryInto<Picture<ValidRef>, Error = PictureError>>,
        F: Fn(VmafStatus) -> (),
    >(
        mut self,
        reference: I,
        ...
        if ref_frames != dist_frames {
            return Err(VmafError::FrameCount(ref_frames, dist_frames));
        }
        ...
        if ref_resolution != dist_resolution {
            return Err(VmafError::Resolution(
            ...
            .map(|(index, result)| match result {
                Ok((reference, distorted)) => {
                    match self.read_pictures(reference, distorted, index.try_into().unwrap()) {
                        Ok(()) => Ok(index),
                        Err(e) => Err(VmafError::Other),
                    }
                }
                Err(error) => Err(VmafError::Other),
            })
            .collect::<Vec<Result<usize, VmafError>>>();

        self.finish_reading_pictures()
            .map_err(|e| VmafError::ClearFrame)?;
